$rootDir = "c:\Users\dance\zone\icons"
$extensions = "*.png", "*.jpg", "*.jpeg" # SVG excluded
$outputFile = Join-Path $rootDir "quiz_images.html"
$quizCount = 10 # Number of questions

# Get all image files
Write-Host "Searching for images..."
$images = Get-ChildItem -Path $rootDir -Include $extensions -Recurse -File
...
foreach ($img in $selectedImages) {
    $relativePath = $img.FullName.Substring($rootDir.Length).TrimStart('\').Replace('\', '/')
    ...
    $safePath = $encodedParts -join '/'
    
    $filename = $img.Name
    # Escape quotes for JS (just in case, though EscapeDataString handles most)
    $safeName = $filename.Replace("'", "\'")
    $jsDataArray += "{ path: '$safePath', name: '$safeName' }"
}
...
<body>
    ...
    <script>
        ...
        function loadQuestion(index) {
            ...
            pathEl.textContent = decodeURIComponent(q.path); // Display decoded path
            ...
            btnNext.disabled = true; // Force user to see answer first? Optional. Let's allow skipping.
        ...
        function updateProgress() {
            // Use backticks and escape dollar sign for PowerShell
